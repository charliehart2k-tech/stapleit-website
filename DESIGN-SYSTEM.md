...
- IT Support packages read left-to-right as `Sole trader`, `Basic`, `Standard`, `Premium` on desktop and in that same order when stacked. The four-column desktop composition becomes two columns on tablet and one column on phone; titles use a contained tier scale and must never collide with the icon or badge.
...
- Add-on pack cards keep concise summaries on the page and expose the full supplied scope in the shared accessible dialog. Long pack content uses a two-column group grid on wide screens and one column on narrow screens; headings and caveats must wrap without clipping.
