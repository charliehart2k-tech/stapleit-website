...
The IT Support hero video is deliberately scaled beyond the pane so the supplied circular form reads as an immersive liquid field rather than a complete floating blob. The `Getting started is simple` chapter uses three neutral black-glass cards over restrained skewed Staple-palette blades: orange/red, blue/purple and Support Green/blue. Each step remains one readable glass pane with no detached decorative glass chips; hover uses only a restrained lift and travelling sheen. The chapter uses Manrope, requires no additional JavaScript and removes motion when reduced motion is requested.
