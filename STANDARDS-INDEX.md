...
The temporary holding page may reuse the approved liquid-wave video behind one full-viewport black-glass composition. Only the word `makeover` uses the restrained Staple-palette moving fill; reduced-motion and forced-colour fallbacks are mandatory. The top-corner private-preview disclosure authenticates server-side with an existing WordPress account, requires the site-builder capability, rate-limits failures and never stores or handles a separate holding-page password.
