...
[[ "$(repo_git rev-parse --is-inside-work-tree 2>/dev/null || true)" == "true" ]] || fail "Git work tree not found at $REPO"
...
grep -Fq 'Recommended for teams of five or more' "$THEME/static-it-support.php"
...
grep -Fq 'Everything is confirmed in a written proposal and service agreement before you commit to anything' "$THEME/static-it-support.php"
grep -Fq 'Every package includes reactive support and proactive monitoring as standard' "$THEME/static-it-support.php"
...
grep -Fq "wp_ajax_nopriv_stapleit_contact_enquiry" "$THEME/functions.php"
grep -Fq "wp_ajax_nopriv_stapleit_cora_chat" "$THEME/functions.php"
grep -Fq "wp_ajax_nopriv_stapleit_cora_status" "$THEME/functions.php"
grep -Fq "stapleit_cora_public_ready" "$THEME/functions.php"
...
grep -Fq "function stapleit_cora_grounded_ready" "$THEME/cora-provider.php"
...
grep -Fq 'data-cora-flow="package"' "$THEME/static-it-support.php"
if grep -Fq 'data-package-finder' "$THEME/static-it-support.php" || grep -Fq 'data-package-question' "$THEME/static-it-support.php"; then
  echo "Legacy package questionnaire returned; refusing deployment." >&2
  exit 1
fi
grep -Fq 'data-cora-open' "$THEME/static-it-support.php"
